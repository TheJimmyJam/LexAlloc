...
// ── Per-page chrome ───────────────────────────────────────────────────────────
function chrome(doc, matterName, totalPages, logoDataUrl) {
...
// ── Main export ───────────────────────────────────────────────────────────────
export async function generateMatterSummaryReport({ matter, parties, insurerPeriods, invoices, financialRows }) {
  ...
  // Logo mark
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', 10, 11, 18, 18)
  } else {
  ...
  for (let i = 1; i <= totalPages; i++) {
    doc.setPage(i)
    chrome(doc, name, totalPages, logoDataUrl)
  }
