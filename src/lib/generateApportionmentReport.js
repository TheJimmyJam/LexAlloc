...
function addPageChrome(doc, matterName, pageCount, logoDataUrl) {
  ...
  // Logo mark in header bar
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', 3, 1.5, 7, 7)
  }

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(7)
  doc.setTextColor(...WHITE)
  doc.text('LexAlloc', logoDataUrl ? 12 : 10, 6.8)
...
export async function generateApportionmentReport(apport) {
  ...
  // Logo mark (real image if available, fallback to drawn placeholder)
  if (logoDataUrl) {
    doc.addImage(logoDataUrl, 'PNG', 10, 12, 18, 18)
  } else {
  ...
  for (let i = 1; i <= totalPages; i++) {
    doc.setPage(i)
    addPageChrome(doc, matterName, totalPages, logoDataUrl)
  }
