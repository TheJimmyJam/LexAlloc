...
//                                              ↘ dupe  (on save)
const STEPS = [
  { key: 'upload', label: 'Upload',   active: ['uploading'],        done: ['parsing','ready','saving','saved','dupe'] },
  { key: 'parse',  label: 'AI Parse', active: ['parsing'],          done: ['ready','saving','saved','dupe'] },
  { key: 'review', label: 'Review',   active: ['ready','dupe'],     done: ['saving','saved'] },
...
export default function InvoiceUploadModal({ matterId, onClose }) {
  ...
  const doneCount    = queue.filter(i => ['saved','error','dupe'].includes(i.status)).length
